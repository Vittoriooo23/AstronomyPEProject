...
public class EarthAsteroidC extends Main{
    ...
    void runCalculations(MouseEvent event) {
        if(Double.parseDouble(massTF.getText()) > 0 && Double.parseDouble(distanceTF.getText()) > 0) {
            totalPE = -1 * G * EarthMassx24 * Double.parseDouble(massTF.getText()) / (Double.parseDouble(distanceTF.getText()) + 6371);
            SPE = -1 * G * EarthMassx24 * Double.parseDouble(massTF.getText()) / 6371;
            for(int i = 0; i < 24; i++)
               totalPE = totalPE * 10;
